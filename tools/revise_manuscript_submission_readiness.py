...
def paragraph_updates() -> dict[int, str]:
    ...
    return {
        ...
        94: "A returned incumbent consists of one repaired legal node and one allocated CPU-cycle rate per task.",
        95: "These characteristics motivate population search over legal categorical assignments and bounded allocated CPU-cycle-rate decisions; no deployment claim is made.",
        ...
        115: "For N tasks, population P, iterations T_max, average legal-node count L and refinement cost R, initialisation is O(PN) plus O(16N) greedy tests, role updates are O(PN), decoding/evaluation is approximately O(PNL), and repair is O(NL) per pass. The practical total is O(PN+T_max PNL+RNL), with O(PN+NL) space for population and decoded legal/capacity state. This implementation-oriented bound is not an optimality guarantee.",
