...
REPLY_FRAGMENT = "已按意见保留并前置 RDHO 作为本文特色"
...
def verify(source: Path, reviewed: Path, clean: Path, audit: Path) -> dict[str, object]:
    with zipfile.ZipFile(source) as base_zip, zipfile.ZipFile(reviewed) as reviewed_zip:
        ...
        for required_text in (
            ...
            "RDHO significantly outperforms RIME but is significantly outperformed by DBO",
            "Equal-NFE population-stage control (Experiment A)",
            "Common-pipeline control (Experiment B)",
            "integrated-framework level",
        ):
